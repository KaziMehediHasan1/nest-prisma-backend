...
export class ProfileVerificationService {
  ...
  async sendVerificationRequest(
    rawData: CreateVerificationRequestDto,
    ...
    if (!rawData.idCard || !rawData.tradeLicense) {
      throw new BadRequestException(
        'Please upload both id card and trade license.',
      );
    }
  ...
  async getAllVerificationRequests({}: PaginationDto): Promise<
    ApiResponse<any>
  > {
    const submissionsWithPayment =
      await this.dbService.verificationSubmission.findMany({
        where: {
          payment: {
            isNot: null,
          },
